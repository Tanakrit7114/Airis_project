export function hydrateMessage(message:any){
 ...
 return {...message,done:true,sources:meta.sources||[],persistent_documents:meta.persistent_documents||[],
  ...
  attachment:meta.attachment_filename,
  attachmentDownloadUrl:meta.document_id ? `/api/documents/${encodeURIComponent(meta.document_id)}/download` : undefined,
  error:meta.error}
}
...
export function updateResponse(messages:any[],id:string,event:any){
 return messages.map(message=>{
  ...
  if(event.type==='error')return {...message,done:true,error:true,content:[message.content,event.message||'An error occurred. Please try again.'].filter(Boolean).join('\n\n')}
